...
#######------- Load Data -------#######
linreg <- read.csv("interpolation_mosquito_com_v2_2013-2016.csv")
...
write.csv(df, "interpolation_mod_mosquito_com_v2_2013-2016.csv", row.names=FALSE)
...
write.csv(corr_mat2, "corr_mat2_com_v2_2013-2016_CV.csv", row.names=FALSE)
...
# Lag0
linreg_X <- linreg[,3:33]
t <- length(linreg$Y)
subsets.1 <- list()
for (n in 1) {
  subset1.1 <- linreg$pred_date[1:t]
...
subsets <- list()
for (n in 1:6) {
  subset1 <- linreg$pred_date[1:(t-n)]
...
#######------- Lag Input -------#######
# Chop off and only get outbreaks
...
data <- lag0 # change this for different lags
outbreak2013 <- subset(data, date>=as.Date('2013-02-03') & date<=as.Date('2013-05-12'))
outbreak2015 <- subset(data, date>=as.Date('2015-02-22') & date<=as.Date('2015-05-03'))
outbreak2016 <- subset(data, date>=as.Date('2015-12-27') & date<=as.Date('2016-05-22'))
outbreaks <- rbind(outbreak2013,outbreak2015,outbreak2016)

my_model <- outbreaks # change this for different lags
# x7,x8,x9,x12,x14,x15,x16,x21,x26 have all 0s, so omitting these predictors below
my_model2 <- my_model[-c(9,10,11,13,15,16,17,22,27)]
...
smylm1.1 <- train(Y~X1+X33, data=my_model2, method="lm", trControl=train.control)
summary(smylm1.1)
...
smylm2.1 <- train(Y~X1++X2+X28+X33, data=my_model2, method="lm", trControl=train.control)
summary(smylm2.1)
...
smylm3.1 <- train(Y~X1+X2+X3+X4, data=my_model2, method="lm", trControl=train.control)
summary(smylm3.1)
...
format(pcr_sorted_mat2, scientific=FALSE)
